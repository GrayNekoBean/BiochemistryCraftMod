				...
				for(int y1 = -1; y1 <= 1; y1++){
					for(int z1 = -1; z1 <= 1; z1++){
						...
						if(worldObj.getBlock(x2, y2 - 1, z2).getMaterial().isSolid() && worldObj.getBlock(x2, y2 - 1, z2) != BCCRegisterBlock.acid){
							...
							FMLLog.log(BiochemistryCraft.NAME, Level.DEBUG, "Pos " + x2 + "," + y2 + "," + z2 + " is set to acid");
						}
						else if (this.isPlants(worldObj.getBlock(x2, y2 - 1, z2))) {
							worldObj.setBlock(x2, y2 - 1, z2, acid);
						}
						else{
						    FMLLog.log(BiochemistryCraft.NAME, Level.DEBUG, "Pos " + (x2) + "," + (y2 - 1) + "," + (z2) + " is not solid");
						}
					    }
					    else{
						FMLLog.log(BiochemistryCraft.NAME, Level.DEBUG, "Pos " + (x2) + "," + (y2) + "," + (z2) + " is not air");
					    }
					}
				}
			}
